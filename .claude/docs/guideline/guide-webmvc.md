...
REST API 프로젝트의 `{table}{CRUD}{###}` 패턴을 따르지 않는다. 현행 명명 기준:
...
- **적용 대상**: Web MVC(WAR) 프로젝트의 정적 리소스 (`common.md` §1 기술 스택 표 참조)
- **핵심 사항**: 패턴별 JS 구조는 `mvc-pattern.md` §4 참조 (전역 함수 패턴·jQuery 플러그인 패턴·객체 리터럴 패턴). 프로젝트별 스코프→패턴 매핑은 **프로젝트 프런트엔드 가이드** 참조
...
- **정적 리소스 경로**: `src/main/webapp/static/`
- **템플릿 경로**: `src/main/webapp/WEB-INF/templates/`
...
`/code-review`·`/mr-review` 가 본 가이드 대상 백엔드 변경을 리뷰할 때 적용하는 유형별 평가 항목. (프론트엔드 평가는 §9 + `frontend-patterns.md` 참조)
