...
> **적용 대상:** 단일 서비스 프로젝트 기본 가이드 (Excel 파일 생성·처리 등)
> **유형:** Spring Boot 기반 Excel 파일 생성·처리 서비스 (Apache POI)
> **단일 출처:** `.claude/config/project.yaml` `projects[]` (`guideline.backend: guide-single.md`)

---

## 1. 패키지 구조

기본 패키지는 `{{config.baseNamespacePattern}}.excel` 이며, 처리 유형별로 패키지를 구성한다.

```
{{config.baseNamespacePattern}}.excel
...
`/code-review`·`/mr-review` 가 본 가이드 대상(Excel) 변경을 리뷰할 때 적용하는 유형별 평가 항목.

<type-eval>

- POI 리소스 `try-with-resources` 닫기
- `SXSSFWorkbook` 대용량 처리
