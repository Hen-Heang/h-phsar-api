...
> **적용 대상:** Redis 캐시·세션 관리 단일 서비스 프로젝트 (project.yaml `projects[].guideline.backend: guide-redis.md` 로 본 가이드 지정)
...
기본 패키지는 `{{config.baseNamespacePattern}}.redis` 이며, 기능 역할별로 패키지를 구성한다.

```
{{config.baseNamespacePattern}}.redis
...
`/code-review`·`/mr-review` 가 본 가이드 대상(Redis) 변경을 리뷰할 때 적용하는 유형별 평가 항목.
