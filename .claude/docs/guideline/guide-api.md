...
모든 API 모듈의 기본 패키지는 `{{config.baseNamespacePattern}}.api.{module}` 이며, 아래 구조를 따른다. (`{{config.baseNamespacePattern}}` = `project.yaml` `baseNamespacePattern` 단일 출처)

```
{{config.baseNamespacePattern}}.api.{module}
  ├── controller/
  ├── service/
  │   ├── impl/
  │   ├── helper/
  ...
  ├── handler/
  ├── filter/
  ├── constants/      ← 외부 API 전용 상수만 (예: ExternalApiResponse)
  └── util/
```
...
D = DELETE : userInfoD001(UserInfoD001In in)                  → int
```
...
매칭 예:
  ...
  orderLedgerC001(OrderLedgerC001In)    → int
  userLedgerR003(UserLedgerR003In)      → List<UserLedgerR003Out>
```
...
- **위치**: `{{config.baseNamespacePattern}}.api.common.templates.ResponseTemplate`
...
`/code-review`·`/mr-review` 가 본 가이드 대상 변경을 리뷰할 때 적용하는 유형별 평가 항목. severity 코드(`[W..]`/`[S..]`)는 `.claude/skills/code-review/references/severity-rules.md` 단일 출처.

<type-eval>

- DTO 네이밍: `{table}{CRUD}{###}`
- Factory 패턴 null 반환 검증 [W12]
- Mapper 메소드명 패턴 [S06]
- `ResponseTemplate` 사용, `ResponseCode`/상수 위치
- Lombok DTO 규칙 [W10]:
  - `*Req` (HTTP 요청 DTO): `@Getter` + `@Setter` + `@NoArgsConstructor` + `@ToString` 필수, `@Builder` 금지
  - `*In` (MyBatis 입력 DTO): `@Getter` + `@Builder` + `@ToString` + `@Alias` 필수, `@Setter` 금지
  - `*Res` (HTTP 응답 DTO): `@Getter` + `@Builder` + `@ToString` + `@JsonInclude(NON_NULL)` 필수, `@Setter` 금지
  - `*Out` (MyBatis 출력 DTO): `@Getter` + `@Setter` + `@ToString` + `@Alias` 필수, `@Builder` 금지
  - `@Alias` 값이 lowerCamelCase 클래스명과 일치하는지 확인
