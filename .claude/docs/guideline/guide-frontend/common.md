...
> **적용 대상:** Web MVC(WAR) + Spring Boot Web(JAR) 프로젝트의 정적 리소스
> **단일 출처:** `.claude/config/project.yaml` `projects[]` (guideline.backend: guide-webmvc.md | guide-webfullstack.md)
>
> 프로젝트별 JS 코딩 패턴(§4)과 패턴별 오류 처리(§7-1, §7-3)는 별도 파일을 참조한다:
> - `mvc-pattern.md` — WAR 패키징 + jQuery 플러그인 패턴 적용 프로젝트
> - `iife-pattern.md` — IIFE + `UI` 네임스페이스 패턴 프로젝트
>
> 프로젝트별 모듈·AJAX 래퍼·공통 라이브러리는 **프로젝트 프런트엔드 가이드**를 참조한다.

---

## 1. 프론트엔드 기술 스택

> **단일 출처:** `.claude/config/project.yaml` `projects[]` 및 프로젝트 프런트엔드 가이드.

프로젝트별 모듈 인벤토리(jQuery·공통 라이브러리 버전, JS 패턴, 패키징)는 **프로젝트 프런트엔드 가이드**를 참조한다.
...
### JAR 프로젝트 (guide-webfullstack.md)
...
> 프로젝트별 AJAX 래퍼 매핑은 **프로젝트 프런트엔드 가이드**를 참조한다.
> 신규 프로젝트 추가 시 `project.yaml` + 프로젝트 프런트엔드 가이드를 함께 갱신한다.

각 프로젝트는 지정된 AJAX 래퍼(`projectAjax(...)`)를 일관되게 사용한다. 패턴별 상세는 `mvc-pattern.md` §4, `iife-pattern.md` §4를 참조한다.
...
### 7-2. 응답 코드 기반 (공통 AJAX 라이브러리 스코프 — §1 표 참조)
...
프로젝트별 사용 라이브러리 목록(공통 모듈·UI 컴포넌트 API)은 **프로젝트 프런트엔드 가이드**를 참조한다.

### 9-2. 사용 원칙

1. **직접 구현 전 기존 라이브러리를 반드시 확인한다.**
2. 프로젝트 공통 라이브러리가 제공하는 기능을 중복 구현하지 않는다.
3. 프로젝트에 UI 컴포넌트 라이브러리가 있는 경우, 네이티브 `alert()`, `confirm()` 대신 해당 컴포넌트를 사용한다.
...
- [ ] AJAX 호출이 프로젝트별 지정 AJAX 래퍼(프로젝트 프런트엔드 가이드 참조)를 따르는가
...
- [ ] 직접 구현 전 프로젝트 공통 라이브러리(프로젝트 프런트엔드 가이드 참조)를 확인했는가
