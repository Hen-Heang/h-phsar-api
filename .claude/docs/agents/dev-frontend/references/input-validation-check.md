---
name: input-validation-check
description: dev-frontend 4-1단계 진입 시점에 Read — HTML/JS 입력 필드 검증 자체 점검 (한국 도메인 정규식·3중 검증 시점·프로젝트 검증 라이브러리 분기·점검 항목).
---

# 4-1단계: 입력 필드 검증 자체 점검

생성한 화면에 *형식이 정해진 입력 필드*가 있으면 **즉시 아래 절차로 검증 룰 적용을 자체 점검**한다.

> **출처 구분**: 본 4-1단계의 한국 도메인 정규식 표(4-1-1)·검증 시점 3중 적용(4-1-2)·점검 항목(4-1-4)은 **가이드 미수록 영역**으로 본 에이전트가 임시 단일 출처 역할을 한다. `guide-frontend/common.md` _폼 유효성 검사_ 섹션이 보강되면 본 4-1단계는 가이드 참조 의무화로 전환 예정.
>
> **운영 사례**: 핸드폰번호 입력 필드를 만들면서 형식 검증을 하나도 적용하지 않은 사례 발생. 본 자체 점검 단계는 그 재발을 차단한다.

## 4-1-1. 한국 도메인 필수 검증 필드 (에이전트 고유 룰 — 가이드 미수록)

| 필드 유형    | HTML5 native                                                         | JS 정규식                               | 표시 형식                              |
| ------------ | -------------------------------------------------------------------- | --------------------------------------- | -------------------------------------- |
| 핸드폰번호   | `type="tel"` `pattern="01[016789]-?\d{3,4}-?\d{4}"` `maxlength="13"` | `/^01[016789]-?\d{3,4}-?\d{4}$/`        | `010-1234-5678`                        |
| 주민등록번호 | `pattern="\d{6}-?\d{7}"` `maxlength="14"`                            | `/^\d{6}-?\d{7}$/` (체크섬 권장)        | `900101-1234567` (뒷자리는 보안 입력 컴포넌트) |
| 사업자번호   | `pattern="\d{3}-?\d{2}-?\d{5}"` `maxlength="12"`                     | `/^\d{3}-?\d{2}-?\d{5}$/` (체크섬 권장) | `123-45-67890`                         |
| 법인번호     | `pattern="\d{6}-?\d{7}"` `maxlength="14"`                            | `/^\d{6}-?\d{7}$/`                      | `110111-1234567`                       |
| 우편번호     | `pattern="\d{5}"` `maxlength="5"`                                    | `/^\d{5}$/`                             | `12345`                                |
| 카드번호     | `pattern="[\d-]{16,19}"` `maxlength="19"`                            | `/^\d{4}-?\d{4}-?\d{4}-?\d{4}$/`        | `1234-5678-9012-3456`                  |
| 이메일       | `type="email"` `maxlength="100"`                                     | `/^[\w.-]+@[\w.-]+\.\w{2,}$/`           | `user@example.com`                     |
| 금액         | `inputmode="numeric"` `pattern="[\d,]+"`                             | 숫자만 추출 후 정수 검증                | `1,000,000`                            |
| 날짜         | `type="date"` 또는 `pattern="\d{4}-\d{2}-\d{2}"`                     | `/^\d{4}-\d{2}-\d{2}$/`                 | `2026-05-06`                           |

## 4-1-2. 검증 시점 (3중 적용)

1. **HTML5 native** — 브라우저 기본 검증 (`pattern`·`required`·`maxlength`·`minlength`·`type`)
2. **입력 중 (onblur 또는 input 이벤트)** — JS 정규식 형식 검증, 실패 시 인라인 에러 메시지 + 필드 색상 변경
3. **제출 직전 (submit 이벤트)** — 전체 필드 일괄 검증, 실패 시 첫 번째 실패 필드로 포커스 이동 + `event.preventDefault()` 또는 `return false`로 폼 제출 차단

## 4-1-3. 프로젝트별 검증 라이브러리

- **프로젝트 폼 검증 유틸 보유 시**: 프로젝트 폼 검증 유틸 사용 (프런트 가이드 또는 `guide-frontend/common.md` 공통 라이브러리 사용 원칙 참조)
- **프로젝트 폼 검증 유틸 미보유 시**: 직접 정규식 검증 함수 작성
- 프로젝트별 정확한 사용법은 `guide-frontend/common.md`의 _공통 라이브러리 사용 원칙_ 섹션 참조

## 4-1-4. 점검 항목

| 점검 항목               | 체크 내용                                                                        |
| ----------------------- | -------------------------------------------------------------------------------- |
| 형식 필드 검증 적용     | 핸드폰·이메일·주민번호 등 형식 정해진 필드에 `pattern` 또는 JS 정규식 적용했는가 |
| `required` 속성         | 필수 입력 필드에 `required` 또는 JS 검증 적용했는가                              |
| `maxlength`/`minlength` | 길이 제한 필드에 적용했는가                                                      |
| 검증 시점 3중 적용      | HTML5 native + 입력 중 + 제출 직전 모두 적용했는가                               |
| 에러 메시지 표시        | 검증 실패 시 어떤 항목이 잘못됐는지 사용자에게 명확히 표시했는가                 |
| 포커스 이동             | 제출 시 첫 번째 실패 필드로 포커스 이동했는가                                    |
| 폼 제출 차단            | 검증 실패 시 폼 제출을 차단했는가                                                |

검증 룰 누락이 발견되면 **수정 전/후를 5단계 보고에 포함**한다.
